...
def isfield(struct, field):
    return field in dir(struct)
...
def init_opts(opts):
    ...
    MINIMUM_TOLERANCE     = np.finfo(float).eps * 100
    ...
    if isfield(opts,'init'):
        ...
        if (not isfield(opts,'W0')) and (opts.init==1):
    ...
    if isfield(opts,'tFlag'):
        if opts.tFlag<0:
            ...
            opts.tFlag=np.floor(opts.tFlag)
    else:
        opts.tFlag = DEFAULT_TERMINATION_COND

    opts.pFlag = DEFAULT_PARALLEL_SWITCH
    # ## Parallel Options (DISABLED)
    # if isfield(opts, 'pFlag'):
    #     if opts.pFlag == True and not exist('matlabpool', 'file'):
    #         opts.pFlag = False
